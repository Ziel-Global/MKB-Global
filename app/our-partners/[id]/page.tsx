...
export default async function CaseStudyDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const hideDetailImages = study.hideDetailImages === true;
    const hasCustomSectionImages = Boolean(study.sectionImages);
    const problemImage = hasCustomSectionImages
        ? study.sectionImages?.problem
        : "/caseStudiesdetails/one.jpg";
    const solutionImage = hasCustomSectionImages
        ? study.sectionImages?.solution
        : "/caseStudiesdetails/two.jpg";
    const resultsImage = hasCustomSectionImages
        ? study.sectionImages?.results
        : "/caseStudiesdetails/three.jpg";
    const imageOnlyDetails = study.imageOnlyDetails === true;
        ...
        <div className="min-h-screen flex flex-col pt-0 bg-white font-sans text-gray-900 selection:bg-[#8B3DFF] selection:text-white">
            ...
            <main className="w-full flex-1 pt-24 md:pt-32 pb-20">
                ...
                <CaseStudyHero
                    ...
                    hideTitle={imageOnlyDetails}
                    fullImage={imageOnlyDetails}
                    hideImage={hideDetailImages}
                />

                {imageOnlyDetails ? null : (
                    <>

                {/* ── Problem section ── */}
                <CaseStudySection
                    title="Problem"
                    imageSrc={hideDetailImages ? undefined : problemImage}
                >
                    {details ? (
                        <>
                            {details.challenge.map((paragraph) => (
                                <p key={paragraph}>{paragraph}</p>
                            ))}

                            {details.challengeBullets && details.challengeBullets.length > 0 && (
                                <ul className="list-disc pl-6 space-y-2">
                                    {details.challengeBullets.map((item) => (
                                        <li key={item}>{item}</li>
                                    ))}
                                </ul>
                            )}
                ...
                <CaseStudySection
                    title="Solution"
                    imageSrc={hideDetailImages ? undefined : solutionImage}
                >
                    {details ? (
                        <>
                            {details.solutionIntro && <p>{details.solutionIntro}</p>}

                            {details.solutionCapabilities && details.solutionCapabilities.length > 0 && (
                                <>
                                    <p>Key capabilities used:</p>
                                    <ul className="list-disc pl-6 space-y-2">
                                        {details.solutionCapabilities.map((item) => (
                                            <li key={item}>{item}</li>
                                        ))}
                                    </ul>
                                </>
                            ...
                            {details.solutionAdditionalModules && details.solutionAdditionalModules.length > 0 && (
                                <>
                                    <p>Additional modules:</p>
                                    <ul className="list-disc pl-6 space-y-2">
                                        {details.solutionAdditionalModules.map((item) => (
                                            <li key={item}>{item}</li>
                                        ))}
                                    </ul>
                                </>
                ...
                <CaseStudySection
                    title="Results"
                    imageSrc={hideDetailImages ? undefined : resultsImage}
                >
                    {details ? (
                        <>
                            <ul className="list-disc pl-6 space-y-2">
                                {details.impactBullets.map((item) => (
                                    <li key={item}>{item}</li>
                                ))}
                            </ul>
                            {details.conclusion && <p className="mt-4">{details.conclusion}</p>}
                        </>
                    ) : (
                        <>
                            <ul className="list-disc pl-6 space-y-2">
